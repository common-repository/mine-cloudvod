  ...
  class MCSF_Shortcoder extends MCSF_Abstract{
    ...
    public function __construct( $key, $params = array() ) {
      ...
      if ( ! empty( $this->args['show_in_editor'] ) ) {
        ...
        MCSF::$shortcode_instances[] = wp_parse_args( array( 'name' => 'csf/'. $name, 'modal_id' => $this->unique ), $this->args );

        // elementor editor support
        if ( MCSF::is_active_plugin( 'elementor/elementor.php' ) ) {
          add_action( 'elementor/editor/before_enqueue_scripts', array( 'MCSF', 'add_admin_enqueue_scripts' ) );
          add_action( 'elementor/editor/footer', array( 'MCSF_Field_icon', 'add_footer_modal_icon' ) );
      ...
      if ( ! empty( $shortcode_key ) && wp_verify_nonce( $nonce, 'csf_shortcode_nonce' ) ) {
        ...
        if ( ! empty( $section ) ) {
          ...
          if ( ! empty( $repeatable_fields ) ) {
            ...
            $button_title    = ( ! empty( $section['button_title'] ) ) ? ' '. $section['button_title'] : esc_html__( 'Add New', 'mine-cloudvod' );
        ...
        echo '<div class="csf-field csf-error-text">'. esc_html__( 'Error: Invalid nonce verification.', 'mine-cloudvod' ) .'</div>';
      }
    ...
    public static function once_editor_setup() {
      ...
      if ( function_exists( 'register_block_type' ) ) {
        add_action( 'enqueue_block_editor_assets', array( 'MCSF_Shortcoder', 'add_guteberg_blocks' ) );
      }
    ...
    public static function add_guteberg_blocks() {
      ...
      wp_enqueue_script( 'csf-gutenberg-block', MCSF::include_plugin_url( 'assets/js/gutenberg.js' ), $depends );

      wp_localize_script( 'csf-gutenberg-block', 'csf_gutenberg_blocks', MCSF::$shortcode_instances );

      foreach ( MCSF::$shortcode_instances as $block ) {
    ...
    public static function add_media_buttons( $editor_id ) {
      ...
      foreach ( MCSF::$shortcode_instances as $value ) {
